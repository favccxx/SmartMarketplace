...
public class DateUtil {
	...
	/**
	 * 获取某一天的凌晨时间（yyyy-MM-dd 00:00:00）
	 * @param str
	 * @return
	 */
	public static Date parseStartDate(Object str) {
		if(str == null) {
			return null;
		}
		
		DateFormat cdf = new SimpleDateFormat("yyyy-MM-dd 00:00:00");
		try {
			return cdf.parse(str.toString());
		} catch (ParseException e) {
			e.printStackTrace();
			return null;
		}
	...
	/**
	 * 获取某一天的半夜时间（yyyy-MM-dd 23:59:59）
	 * @param str
	 * @return
	 */
	public static Date parseEndDate(Object str) {
		if(str == null) {
			return null;
		}
		
		DateFormat cdf = new SimpleDateFormat("yyyy-MM-dd 23:59:59");
		try {
			return cdf.parse(str.toString());
		} catch (ParseException e) {
			e.printStackTrace();
